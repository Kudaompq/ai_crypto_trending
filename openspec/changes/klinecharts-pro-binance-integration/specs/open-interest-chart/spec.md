...
The system SHALL provide an open-interest study that can be enabled in a secondary chart pane and SHALL request historical samples from Binance USDⓈ-M for the currently selected trading pair and chart interval. The study SHALL plot the Binance `sumOpenInterest` quantity field, not the `sumOpenInterestValue` notional-value field. Its label SHALL identify open-interest quantity rather than value. The open-interest study selection SHALL be shared by all trading pairs in the current browser and SHALL be reused across all chart intervals. Samples SHALL be plotted only at timestamps and quantity values returned by Binance; the system SHALL NOT interpolate, carry forward, or replace missing samples with another data source.

#### Scenario: Show available Binance samples
- **WHEN** the user enables open interest and Binance returns samples whose `sumOpenInterest` and `sumOpenInterestValue` fields differ
- **THEN** the secondary pane plots `sumOpenInterest` at the returned timestamps and does not plot `sumOpenInterestValue`
...
- **THEN** the pane contains quantity values only where samples were returned and leaves the unavailable range empty
...
- **THEN** open interest remains enabled for the other symbol while its quantity samples are requested for that selected symbol

#### Scenario: Keep open-interest selection across intervals
- **WHEN** the user changes the chart interval
- **THEN** open interest remains enabled and the system requests quantity samples for the newly selected interval
