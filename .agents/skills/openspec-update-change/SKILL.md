...
`$openspec-continue-change (Codex) or /openspec-continue-change (other agents)` 是一个扩展 profile 工作流，可能未安装。在下方任何地方建议它之前，请验证它是否可用。若不可用，`openspec-cn status --change "<name>" --json` 显示下一个制品，`openspec-cn instructions "<artifact-id>" --change "<name>" --json` 解释如何创建它。
...
- 在写入前与用户确认每个编辑。
- 若请求更改的是变更的*意图*而非细化，首先验证扩展 profile `$openspec-new-change (Codex) or /openspec-new-change (other agents)` 工作流是否可用。若可用，建议用 `$openspec-new-change (Codex) or /openspec-new-change (other agents)` 重新开始（"更新 vs 重新开始" 启发式）。若不可用，请求一个不同的未使用变更名称并建议改用 `openspec-cn new change "<new-change-name>"`。
